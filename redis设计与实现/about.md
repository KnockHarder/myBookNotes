...
- 通过`MULTI`命令我们可以批量提交命令
	- 服务器在接收到`MULTI`后，将客户端置为MULTI模式
	- 后续来的命令，将被添加到待执行列表中，不会实际执行
	- 直到服务器接收到`EXEC`命令，此时才列表中的命令按序执行
- 有部分命令即使开启了 MULTI 模式也会立即执行: multi、exec、discard、watch
	- 在 MULTI 模式下再次执行`MULTI`命令，由于MULTI命令无法相互嵌套，所以会立即得到执行失败的返回
	- discard命令的作用是终止本次事务
	- watch命令的作用是监视当前数据库的某些键，当其值变化后，会将客户端标记为 dirty cas 状态。该命令必须在 MULTI 模式下才能启用。
- 在执行`EXEC`命令时，如果客户端是否处于 dirty cas 状态或者 dirty exec 状态，会放弃执行本次事务，并返回错误信息
- dirty exec 状态: 当命令参数不合法或服务器状态异常时，会拒绝执行命令，此时 MULTI 模式下的命令不会进入待执行队列，且客户端置为 dirty exec 状态。
- 无论事务执行成功或失败，都会重置事务——包括命令、状态重置，以及清空监视列表。
- 由于`SELECT`命令无法被立即执行，而`WATCH`必须在 MULTI 模式下才能使用，另一方面事务结束后会清空监视列表，因此被监听的键值一定是开启事务时客户端所使用的的数据库。
	无法通过`SELECT`命令切换需要监听的数据库，也无法监听多个数据库。
- **需要注意的是**:
	- 事务一旦开始执行，便不会中断，即使某命令执行过程中出现错误也会继续向下执行——这里的错误一定指只有在执行时才能确认的错误（如类型错误），而在命令入队列前的检查没通过的话，
	  整个事务都不会执行。
	- 也不支持回滚操作。命令执时的结果是立即生效的，redis仅是利用了其单线程处理逻辑实现了事务机制，也就无法实现回滚。
...
- 由于初次加入集群时必须为主节点，因此配置主从复制无法像非集群那样直接配置salveof。而是通过`cluster replicate`命令将当前节点变为某主节点的从节点。
	需要注意的是，当前节点的必须未接管分片且数据库为空，因此该命令只适合初始化时使用。
- 由于节点身份的传播依赖于该节点自身发送的消息，因此传播速度会慢一些
- 当主节点处于FAIL状态时，从节点开始尝试进行故障恢复
	- 首先会检查自身是否有资格竞选主节点: `data_age`须小于一次ping循环时间，加上集群节点过期时间的一个倍数（通过`cluster-replica-validity-factor`配置）
	- 然后通过repl_offset进行从大到小排名，排名靠前的节点和master节点的数量最接近，优先请求集群中的各主节点投票
	- 被要求投票的主节点会确认该从节点存储的主节点分片信息是否落后于自己，如果落后拒绝投票；否则投其一票，并记录投票时间与epoch，后续一段时间内不再投票，对于配置落后的节点也拒绝投票。
	- 当某个从节点得到超过一半的主节点的投票时，会被选举为新的主节点
	- 当下线的旧主节点恢复过来时，同步完分片信息后，会将自己设置为新的主节点的从节点
